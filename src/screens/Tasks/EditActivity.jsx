...
function AddSchedule({ route }) {
  const { lead } = route.params || {};  
    ...
    const validateForm = () => {
        const newErrors = {};
        console.log("selected mode ","as "+JSON.stringify(selectedMode));
                    ...
                    setModes(filteredModes);
                     const options = filteredModes.map(mode => ({
                      ...
                      if (lead?.mode_communication) {
                            const matched = filteredModes.find(mode => mode.id === lead.mode_communication);
                            if (matched) {
                                setSelectedMode(matched);

                                // Optional: set form values too if needed
                                setFormValues(prev => ({
                                    ...prev,
                                    mode: matched.id,
                                    modeName: matched.name || '',
                                }));
                            }
                    }
