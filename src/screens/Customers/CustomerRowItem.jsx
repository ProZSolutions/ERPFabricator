...
const ContactCard = ({ details }) => {
  ...
  const toCamelCase = (name) => {
    ...
    const colors = [
      ...
      const label = details.customer_id; // or "Enquiry", "Field Visit"
      ...
      const firstLetter = getInitials(details.name);
    ...
    <View className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 m-3">
      ...
      <View className="flex-row items-center justify-between">
        ...
        <View className="flex-row items-center">
          ...
          <View>
            <Text className="text-black font-semibold text-base text-[12px]">{toCamelCase(details.name)}</Text>
            <Text className={`text-[10px]  text-black`}>{label}</Text>
        ...
        {details?.contact && (
          <View className="items-center">
            <TouchableOpacity className="border border-gray-300 p-2 rounded-full"   onPress={() => handleCall(details.contact)}>
