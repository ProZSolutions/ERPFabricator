    ...
    return (
        <StyledView className="bg-white border border-gray-200 rounded-lg shadow-sm px-2 py-2 mb-4">
            <StyledView className="flex-row justify-between items-center">
                <StyledView>
                    <Text className="text-base text-custom-heading font-medium">{chitNameFormat(item?.chit_name, item?.chit_id)}</Text>
                    <Text className="text-custom-companytxt text-xs font-normal mt-2">{item?.start_month}</Text>
                </StyledView>

                <StyledView className="items-end">
                    <TouchableOpacity onPress={() => navigation.navigate("MyChitDetails", {item:item})} className="flex-row items-center space-x-1">
                        <Text className="text-custom-hyperlink text-xs font-normal">View</Text>
                        <GreaterThanIcon />
                    </TouchableOpacity>
                    <StyledView className="flex-row items-center">
                        <Text className="text-custom-heading font-medium text-base mt-2">{formatAmount(item?.cumulative_amount)}</Text>
                    </StyledView>
                </StyledView>
            </StyledView>
