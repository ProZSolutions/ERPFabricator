...
function MemberPaymentDetailsCard({ obj }) {
    return (
        <View className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 mb-3 pb-3">
            <View className="flex-row pt-1 pb-2">
                ...
                <Text className="text-custom-heading text-xs font-medium">{chitNameFormat(obj?.chit_name, obj?.chit_id)}</Text>
            </View>
            {obj?.payment?.map((item) => (
                <View>
                    ...
                    <View className="flex-row justify-between mb-2">
                        ...
                        <Text className="text-custom-companytxt text-center w-[20%]">:</Text>
                        <Text className="text-custom-heading text-sm font-medium w-[40%]">{item?.payment_mode}</Text>
                    </View>

                    <View className="flex-row justify-between">
                        <Text className="text-custom-companytxt text-xs font-normal w-[40%]">Transaction Details</Text>
                        <Text className="text-custom-companytxt text-center w-[20%]">:</Text>
                        <Text className="text-custom-heading text-sm font-medium w-[40%]">{item?.payment_details || '-'}</Text>
                    </View>
                </View>
            ))

            }
            {obj?.settlement?.map((item) => (
                <View>
                    <View className="border-b border-gray-200 my-4"></View>
...
export default MemberPaymentDetailsCard;