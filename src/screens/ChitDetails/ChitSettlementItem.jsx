...
function ChitSettlementItem({ index, item, isExpanded, onPress, previewModal = () => {} }) {
        ...
        <StyledView className='mb-4'>
            ...
            >
                {/* <StyledView className="basis-[35%]">
                    <StyledText className="text-custom-heading text-base font-medium">{name}</StyledText>
                    <StyledText className={`${id == 1 ? 'text-custom-green' : 'text-custom-hyperlink'
                ...
                <StyledView className="basis-[30%] flex-row justify-end items-center">
                    {item.auction_win_type === 'custom'? <LottUserIcon/> : <LottIcon/>}
                    <DownArrowIcon />
                </StyledView>
            </StyledTouchableOpacity>
            <Collapsible collapsed={!isExpanded}>
                <View className="py-2 rounded-bl-xl rounded-br-xl shadow-md mb-2 border border-gray-200">
                    <StyledView className="px-2">
                        ...
                        {item?.payment_details?.[0]?.document && 
...
export default ChitSettlementItem;