...
function ChitSummaryMembersInfo({ member }) {
  const { id, name, contact, chit_status = null, chit_type = null } = member;
    ...
    <StyledView className="mb-2">
      <StyledTouchableOpacity className="flex flex-row items-center justify-between p-2 border border-gray-200 rounded-lg">
        ...
        <StyledView className="basis-[35%]">
          ...
          <StyledText
            className={`${
                chit_status !== null ? "text-custom-green" : "text-custom-hyperlink"
            } text-xs font-light`}
          >
            {contact}
          </StyledText>
...
export default ChitSummaryMembersInfo;
